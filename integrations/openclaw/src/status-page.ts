...
export function createReMeStatusHandler(options: StatusPageOptions) {
  return async (request: IncomingMessage, response: ServerResponse) => {
    ...
    if (request.method === "GET" && pathname === `${STATUS_PATH}/api/status`) {
      ...
      sendJson(response, payload.reme.connected ? 200 : 503, payload);
      return;
    }
    if (request.method === "POST" && pathname === `${STATUS_PATH}/api/dream`) {
      await options.runtime.runDream();
      sendJson(response, 200, { runtime: options.runtime.snapshot() });
      return;
    }
...
function renderStatusHtml(
  ...
  const values: Record<string, unknown> = {
    health: payload.reme.connected ? "Healthy" : "Offline",
    ...
    ([group, entries]) => {
      ...
      return instances.map(([name, details]) => {
        ...
        return `<div class="component"><strong>${escapeHtml(
          group.replaceAll("_", " "),
        )} · ${escapeHtml(name)}</strong><span class="good">${
          details.is_started === false ? "Stopped" : "Healthy"
        }</span><div class="label">${summary}</div></div>`;
      });
    },
  );
  html = html.replace(
